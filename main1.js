...
function addRow() {
  ...
  + '</td><td>'
  + (lastRowNumebr + 1)
  + '</td><td><input class="exectime" type="text"/><td class="servtime" style="display:none;"></td>'
  + '<td class="priority-only"><input type="text"/></td></tr>';

  $('.servtime').css("display","none");
    ...
    while (!areWeThereYet) {
      ...
      $.each(executeTimes, function (key, value) {
        if (value.executeTime > 0) {
          th += '<th style="height: 50px; width: ' + (value.executeTime > quantum ? quantum : value.executeTime) * 20 + 'px;">P' + value.P + '</th>';
